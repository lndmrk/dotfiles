...
(use-package bind-key
  :config
  (bind-keys :prefix-map my-buffer-prefix-map
             :prefix "C-c b")
  (bind-keys :prefix-map my-compile-prefix-map
             :prefix "C-c c")
  (bind-keys :prefix-map my-config-prefix-map
             :prefix "C-c d")
  (bind-keys :prefix-map my-gptel-prefix-map
             :prefix "C-c i")
  (bind-keys :prefix-map my-hs-prefix-map
             :prefix "C-c t")
  (bind-keys :prefix-map my-macro-prefix-map
             :prefix "C-c x")
  (bind-keys :prefix-map my-multiple-cursors-prefix-map
             :prefix "C-c m")
  (bind-keys :prefix-map my-prog-prefix-map
             :prefix "C-c l")
  (bind-keys :prefix-map my-rewrite-map
             :prefix "C-c r")
  (bind-keys :prefix-map my-search-prefix-map
             :prefix "C-c s")
  (bind-keys :prefix-map my-window-prefix-map
             :prefix "C-c w"))
#+end_src
...
(use-package emacs
  :preface
  (defun my-tab-always-indent-complete-advice (fn &rest args)
...
(use-package emacs
  ...
  (undo-limit (* 640 1000))
  (undo-outer-limit (* 96 1000 1000))
  (undo-strong-limit (* 960 1000))
...
(use-package emacs
  :preface
  (defun my-prose-mode ()
    "Mode suitable for writing prose."
    (interactive)
    (display-line-numbers-mode 0)
    (my-set-fill-column most-positive-fixnum)
    (visual-line-mode 1)))
#+end_src
...
(use-package cc-mode
  ...
  :config
  (advice-add 'c-indent-line-or-region
              :around #'my-tab-always-indent-complete-advice))
#+end_src
...
(use-package display-fill-column-indicator
  :hook
  ((find-file conf-mode prog-mode text-mode) .
...
(use-package display-line-numbers
  ...
  :hook
  ((conf-mode prog-mode text-mode) . my-display-line-numbers-mode))
#+end_src
...
(use-package flyspell
  :hook
  ((conf-mode prog-mode) . flyspell-prog-mode)
...
(use-package hideshow
  :hook
  ((conf-mode prog-mode) . hs-minor-mode)
  :bind
  (:map my-hs-prefix-map
   ("h" . hs-hide-all)
   ("s" . hs-show-all)
   ("t" . hs-toggle-hiding)))
#+end_src
...
#+begin_src emacs-lisp
(use-package subword
  :hook ((conf-mode prog-mode) . subword-mode))
#+end_src
...
(use-package whitespace
  ...
  (whitespace-style '(face
                      ...
                      tab-mark))
  :hook
  ((find-file conf-mode prog-mode text-mode) . whitespace-mode))
#+end_src
...
(use-package corfu
  ...
  (defun my-corfu-move-to-minibuffer ()
    "Move the completions to the minibuffer."
    (interactive)
    (pcase completion-in-region--data
      (`(,beg ,end ,table ,pred ,extras)
       (let ((completion-cycle-threshold completion-cycling)
             (completion-extra-properties extras))
...
(use-package multiple-cursors
  :ensure t
  :bind
  (:map my-multiple-cursors-prefix-map
...
(use-package nhexl-mode
  :ensure t
  :preface
  (defun my-buffer-binary-p ()
    (with-current-buffer (current-buffer)
      (save-excursion
        (goto-char (point-min))
        (search-forward-regexp
         ;; Any ASCII control character except HT, LF, VT or CR.
         (rx (any (?\x00 . ?\x08) "\f" (?\x0e . ?\x1f) "\x7f"))
         4096 'noerror 1))))
  (defun my-nhexl-mode ()
    (nhexl-nibble-edit-mode 1)
    (setq-local require-final-newline nil))
  :init
  ;; Cannot use :magic-fallback since it assumes CAR is a regexp string.
  (add-to-list 'magic-fallback-mode-alist '(my-buffer-binary-p . nhexl-mode))
  :custom
  (nhexl-silently-convert-to-unibyte t)
  :hook
  (nhexl-mode . my-nhexl-mode))
#+end_src
...
(use-package tempel
  ...
  (defun my-tempel-include (elt)
    (pcase-let ((`(i ,name) elt))
      (when-let ((template (alist-get name (tempel--templates))))
        `(l ,@template))))
  :custom
